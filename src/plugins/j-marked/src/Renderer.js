...
module.exports = class Renderer {
  ...
  bilibiliEmoji(text) {
    const href = cleanUrl(this.options.sanitize, this.options.bilibiliEmojiUrl, text + '.png');
  ...
  codeEmoji(text) {
    // 末尾有！为 gif
    const extension = `.${/.+!$/.test(text) ? 'gif' : 'png'}`;
    const name = text.replace(/!$/, '');
    const href = cleanUrl(this.options.sanitize, this.options[`${extension === '.gif' ? 'tieba' : 'haha'}EmojiUrl`], `icon_${name}${extension}`);
